...
VM_LIST='py/Vms.yaml'
BENCHMARK_LIST='py/Benchmarks.yaml'
...
def extract_result_vm(bench, vm, config):
    ...
    for heap in config['heapsize']:
        result_file = ("dats/%d_%d/results/raw/%s_%d_t3_%s.txt" % (config['basebit'], config['target'], vm['name'], heap['size'], bench))
...
#            if benchname == "dht11":
#                sys.stdout.write('%s, %s, %s, %f\n' % (benchname, vm['name'], heap['name'], prog[0][0]))
            if vm_i == 0:
        ...
        ax.bar(left, height, width = width,
                ...
                yerr = (yerr_up, yerr_low), label = vm['label'],
    ...
    yerr_up = []
    yerr_low = []
    for vm_i,(result,_,vmname) in enumerate(data):
        for prog_i,(prog,_,_,heap) in enumerate(result):
            left.append(prog_i + vm_i * width - 0.5 + width)
...
#    anchor = (1, 1)
#    loc = 'upper left'
#    fig.legend(bbox_to_anchor=anchor, loc=loc, borderaxespad=10)
#    fig.tight_layout()
    outfname = 'exectime_%d_%d_%s.eps' % (config['basebit'], config['target'], benchname)
