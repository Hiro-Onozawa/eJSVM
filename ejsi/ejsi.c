...
#ifdef BIT_32
#define DEFAULT_BASEBIT         32
#define DEFAULT_SPECPTAG        "01"
#else
#define DEFAULT_BASEBIT         64
#define DEFAULT_SPECPTAG        "110"
#endif

char *prompt, *ejsvm, *ejsc_jar, *ejsvm_spec, *tmpdir, *specPTAG;
int basebit, verbose;
...
struct commandline_option options_table[] = {
  ...
  { "-b",         1, &basebit,        NULL          },
  { "--basebit",  1, &basebit,        NULL          },
  { "-P",         1, NULL,            &specPTAG     },
  { "--specPTAG", 1, NULL,            &specPTAG     },
...
int process_options(int ac, char *av[]) {
  ...
  specPTAG = DEFAULT_SPECPTAG;
  basebit = DEFAULT_BASEBIT;
  verbose = FALSE;
...
int do_frontend(int pid) {
  ...
  for (i = 0; ; i++) {
    ...
    snprintf(ejsc_command, BUFLEN,
             "java -jar %s -fn %d -O --spec %s --out-obc %s --basebit %d --specPTAG %s",
             ejsc_jar, fn, ejsvm_spec, tmpjs_filename, basebit, specPTAG);
    if (verbose == TRUE)
